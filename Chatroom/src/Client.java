	...
	private class Receiver extends Thread {
		...
		public void run() {
			try {
				...
				if (sender.equalsIgnoreCase("Server") && (received.getValue() != null)) {
		...
		while (chatting) {
			...
			try {
				...
				if (chat_scanner.hasNextLine()) {
					...
					if ((command.equalsIgnoreCase("message") || command.equalsIgnoreCase("private"))
							&& !param.isEmpty()) {
						index = param.indexOf(' ');
...
//							client_Socket.close();
					...
					} else if (command.equalsIgnoreCase("broadcast") && !param.isEmpty()) {
					...
					} else if (command.equalsIgnoreCase("getaddress") && !param.isEmpty()) {
